 ...
 *
 * The example of secret for Google Authentificator:
...
// Analogicallly to the prev. function.
...
static int decode_char(unsigned char c) {
  char retval = -1;
  if (c >= 'A' && c <= 'Z') retval = c - 'A';
  if (c >= 'a' && c <= 'z') retval = c - 'a';
  if (c >= '2' && c <= '7') retval = c - '2' + 26;
  return retval;
}
...
size_t base32_decode(const unsigned char *coded, size_t coded_len, unsigned char *plain) {
  size_t written = 0; // Count for succesfully collected bytes/
